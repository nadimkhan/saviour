...
*Setup instructions for a gitian build of SAVIOUR using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SAVIOUR
...
- [Building SAVIOUR](#building-saviour)
...
Clone the git repositories for saviour and gitian and then checkout the saviour version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/saviour-crypto/saviour.git
cd saviour
...
Follow the instructions in [doc/release-process.md](release-process.md) in the saviour repository
...
To build SAVIOUR (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the saviour repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/saviour/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/saviour-crypto/saviour
...
For example:
```bash
URL=https://github.com/crowning-/saviour.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit saviour=${COMMIT} --url saviour=${URL} ../saviour/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit saviour=${COMMIT} --url saviour=${URL} ../saviour/contrib/gitian-descriptors/gitian-win.yml
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/saviour-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/saviour-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/saviour-build.assert
```
...
[saviour/gitian.sigs](https://github.com/saviour-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
